...
def paw(chat_id):
    try:
        ...
        if response.status_code == 200:
            ...
            bot.send_message(chat_id, msg, reply_markup=keyboardMarkup, parse_mode='markdown')
        else:
            msg = 'Got unexpected status code {}: {!r}'.format(response.status_code, response.json())
            bot.send_message(chat_id, msg, reply_markup=keyboardMarkup, parse_mode='markdown')
    except Exception as e:
        mdg = f'"Exception (weather):", {e}'
        bot.send_message(chat_id, msg, reply_markup=keyboardMarkup, parse_mode='markdown')
...
def wttr_by_location(chat_id, lattitude=lat, longtitude=lon, lang='ru'):
    try:
        url = f'http://wttr.in/{lattitude}%2C{longtitude}_Mmtp_lang={lang}.png'
        response = requests.get(url)
        if response.status_code == 200:
            guid = uuid.uuid4()
            with open(f'./{guid}.png', 'wb') as f:
                f.write(response.content)
            bot.send_photo(chat_id, photo=open(f'./{guid}.png', 'rb'), reply_markup=keyboardMarkup, parse_mode='markdown')
            os.remove(f'./{guid}.png')
        else:
            msg = 'Got unexpected status code {}: {!r}'.format(response.status_code, response.json())
            bot.send_message(chat_id, msg, reply_markup=keyboardMarkup, parse_mode='markdown')
    except Exception as e:
        bot.send_message(chat_id, f'"Exception (weather):", {e}', reply_markup=keyboardMarkup, parse_mode='markdown')
